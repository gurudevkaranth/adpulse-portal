...
export default function AdSetsTab({ campaigns, filters, onFiltersChange }) {
  ...
  const filteredData = activeBucket === 'all' ? adSets : adSets.filter(row => getRowBucket(row) === activeBucket);

  const bucketCounts = useMemo(() => {
    const counts = { winners: 0, bau: 0, iteration: 0 };
    adSets.forEach(row => { counts[getRowBucket(row)]++; });
    return counts;
  }, [adSets]);
