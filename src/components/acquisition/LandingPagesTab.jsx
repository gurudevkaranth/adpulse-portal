...
function ColumnPicker({ columns, visibleColumns, onChange }) {
...
function FilterPopover({ filters, onFiltersChange }) {
  const [open, setOpen] = useState(false);
  const ref = useRef(null);
...
const METRIC_COLUMNS = [
  ...
  { key: 'metrics.revenue', label: 'Revenue', defaultVisible: true, tip: 'Total revenue attributed to this landing page' },
...
function buildColumns(sparklines, showTrends, visibleColumns) {
  const cols = [
    {
      key: 'name', label: 'Page / Ad Name', sortable: true,
      render: (v, row) => {
        if (row.type === 'landingPage') return v;
        return (
  ...
  METRIC_COLUMNS.forEach(mc => {
    ...
    const col = { key: mc.key, label: mc.label, align: 'right', info: mc.tip };
    switch (mc.key) {
      case 'metrics.spend':
        col.sortable = true;
        col.render = (v) => <span className="text-text-primary">{formatCurrency(v || 0)}</span>;
        break;
      ...
      case 'metrics.roas':
        col.sortable = true;
        col.render = (v) => <span className="font-semibold text-text-primary">{formatRoas(v || 0)}</span>;
        break;
      case 'metrics.ctr':
        col.render = (v, row) => <span className="text-text-primary">{formatPercent(v || row.metrics?.ctr || 0)}</span>;
        break;
      case 'metrics.cpa':
        col.render = (_, row) => {
          const m = row.metrics;
      ...
      case 'metrics.conversions':
        col.render = (v) => <span className="text-text-primary">{formatNumber(v || 0)}</span>;
        break;
    }
    cols.push(col);
  });

  // Trend column — only if toggled on
  if (showTrends) {
    cols.push({
...
export default function LandingPagesTab({ data, filters, onFiltersChange }) {
  ...
  const sparklines = useMemo(() => {
    const map = {};
    data.forEach(page => {
      map[page.id] = generateSparklineData(14);
      (page.children || []).forEach(ad => {
        ...
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-base font-semibold text-text-primary flex items-center gap-2">
              ...
              <InfoTip text="See which landing pages drive the best conversions. Click to expand ad details." />
            </h2>
            <p className="text-xs text-text-secondary mt-0.5">{filteredData.length} landing pages</p>
          </div>
          <div className="flex items-center gap-2">
            <GradeLegend />
