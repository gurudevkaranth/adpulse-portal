...
import type { Ad, Grade, LandingPage, SparklineDataPoint } from '../../types';
...
export default function LandingPagesTab({ data, filters, onFiltersChange }: LandingPagesTabProps) {
  ...
  const filteredData = activeBucket === 'all' ? data : data.filter(row => getRowBucket(row as unknown as TableRow) === activeBucket);

  const bucketCounts = useMemo(() => {
    const counts: Record<string, number> = { winners: 0, bau: 0, iteration: 0 };
    data.forEach(row => { counts[getRowBucket(row as unknown as TableRow)]++; });
    return counts;
  }, [data]);
