...
import { generateComparativeData, getScoreColor } from '../../data/mockData';
...
export default function ComparativeTab({ ads }: ComparativeTabProps) {
  const [viewMode, setViewMode] = useState<string>('format');
  const [selectedMetric, setSelectedMetric] = useState<string>('roas');

  const comparativeData: ComparativeCategory[] = useMemo(() => generateComparativeData(), []);
  ...
  // Platform comparison
  const platformComparison: PlatformSummary[] = useMemo(() => {
