...
function generateChannelTableData() {
  const channels = [
    {
      category: 'Paid Ads',
      sources: [
  ...
  function addSparkAndChange(row) {
    const changes = {};
    const sparklines = {};
...
function MetricCell({ value, change, sparklineData, format, showTrends }) {
...
export default function ChannelsTab({ channelChart, filters, onFiltersChange }) {
  const [chartType, setChartType] = useState('area');
  const [selectedMetric, setSelectedMetric] = useState('revenue');
  const [expandedChannels, setExpandedChannels] = useState(new Set(['Paid Ads']));
  const [visibleColumns, setVisibleColumns] = useState(DEFAULT_VISIBLE_COLUMNS);
  const [showTrends, setShowTrends] = useState(false);

  const channelTotals = useMemo(() => {
    return channelChart.channels.map((ch, i) => {
      const revenue = channelChart.data.reduce((s, d) => s + (d[ch] || 0), 0);
  ...
  const sidebarData = useMemo(() => {
    return channelTableData.map((ch, i) => ({
      channel: ch.category,
      value: ch.metrics[sidebarMetricKey] || 0,
      colorIndex: i,
