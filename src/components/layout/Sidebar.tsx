...
const BOTTOM_LINKS: { icon: LucideIcon; label: string }[] = [
...
export default function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  const [expandedGroups, setExpandedGroups] = useState<Record<string, boolean>>({ 'Analyze': true, 'Creative Intel': true, 'Acquisition': true });
  ...
  const isItemActive = (item: NavItem | NavChild): boolean => {
    if (item.to === '/') return location.pathname === '/';
    if (item.tab) {
      const params = new URLSearchParams(location.search);
      return location.pathname === item.to && params.get('tab') === item.tab;
    }
    if ('children' in item && item.children) return item.children.some(c => isItemActive(c));
    return location.pathname.startsWith(item.to);
  };

  const handleNavClick = (e: React.MouseEvent, item: NavItem | NavChild) => {
    if (item.tab) {
      e.preventDefault();
