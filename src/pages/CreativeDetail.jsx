...
export default function CreativeDetail() {
  ...
  const history = useMemo(() => ad ? generateAdPerformanceHistory(ad) : [], [ad]);
  const competitors = useMemo(() => ad ? generateCompetitiveInsights(ad) : [], [ad]);
  const recommendations = useMemo(() => ad ? generateAIRecommendations(ad) : [], [ad]);
  const performanceSummary = useMemo(() => ad ? generatePerformanceSummary(ad) : { strengths: [], weaknesses: [] }, [ad]);
  const creativeAnalysis = useMemo(() => ad ? generateCreativeAnalysis(ad) : null, [ad]);
