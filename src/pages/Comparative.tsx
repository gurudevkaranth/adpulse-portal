...
import type { Ad, ComparativeCategory } from '../types';
...
export default function Comparative() {
  ...
  const comparativeData = useMemo(() => generateComparativeData() as ComparativeCategory[], []);
  ...
  const platformComparison = useMemo((): PlatformComparisonEntry[] => {
    const platforms: Record<string, { platform: string; ads: number; totalSpend: number; totalRevenue: number; totalConversions: number; scores: number[] }> = {};
    allAds.forEach(ad => {
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={filteredData}>
            ...
            <Bar dataKey={selectedMetric} radius={[6, 6, 0, 0]}>
              {filteredData.map((_: ComparativeCategory, i: number) => (
                  ...
                  <tr key={item.label} className="border-b border-border-light hover:bg-gray-50">
                    ...
                    <td className="py-2.5 pl-2 text-right">
                      <span
                        ...
                        style={{ background: getScoreColor(item.hookScore) as string }}
                      >
                        {item.hookScore}
                      </span>
                    </td>
          ...
          <ScatterChart>
            ...
            <Tooltip
              ...
              formatter={((value: number | undefined, name: string) => {
                if (name === 'Spend') return [formatCurrency(value as number), name];
            ...
            <Scatter data={scatterData} fill="#3b82f6" fillOpacity={0.6}>
              {scatterData.map((entry, i) => (
                <Cell key={i} fill={getScoreColor(entry.z) as string} fillOpacity={0.7} />
              ))}
            </Scatter>
          </ScatterChart>
        </ResponsiveContainer>
            ...
            <div key={p.platform} className="border border-border rounded-xl p-4 hover:shadow-md transition-shadow">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-xs items-center">
                  ...
                  <span
                    className="inline-flex items-center justify-center w-7 h-7 rounded-full text-[10px] font-bold text-white"
                    style={{ background: getScoreColor(p.avgScore) as string }}
                  >
                    {p.avgScore}
                  </span>
                </div>
