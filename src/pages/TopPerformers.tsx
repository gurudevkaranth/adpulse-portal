...
import type { Ad, LeaderboardEntry } from '../types';
...
export default function TopPerformers() {
  ...
  const leaderboard = useMemo(() => generateWeeklyLeaderboard(allAds) as LeaderboardEntry[], [allAds]);
  ...
  interface TopMetricEntry {
    name: string;
    value: number;
    score: number;
  }

  const topMetricData = useMemo((): TopMetricEntry[] => {
    return [...allAds]
      .sort((a, b) => {
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={scoreDistribution} layout="vertical">
              ...
              <Tooltip
                ...
                formatter={(value: number | undefined) => [`${value} creatives`, 'Count']}
              />
              <Bar dataKey="count" radius={[0, 4, 4, 0]}>
                {scoreDistribution.map((entry, i) => (
                  <Cell key={i} fill={getScoreColor((entry.min + entry.max) / 2) as string} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
            ...
            <BarChart data={topMetricData} layout="vertical">
              ...
              <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                {topMetricData.map((entry, i) => (
                  <Cell key={i} fill={getScoreColor(entry.score) as string} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
